...
Migrado a PySide6. Estilos delegados al QSS global para unificación B2B.
"""
...
class TaskCard(QFrame):
    def __init__(self, parent, task_data: dict, project_root: Path, is_installed: bool, 
        ...
        # Unificación B2B: Hereda estilos corporativos
        self.setObjectName("FloatingCard")
        
        # Opcional: Control dimensional elástico pero con mínimos
    ...
    def _build_ui(self):
        ...
        # Fondo oscuro puro para las bandas del thumbnail
        ...
        # ---------------------------------------------------------
        # Fila Inferior: Botones de Acción (Deep Linking)
        # ---------------------------------------------------------
        ...
        if task_url:
            ...
            self.kitsu_btn.clicked.connect(lambda _, u=task_url: webbrowser.open(u))
        ...
        if not self.project_root:
            ...
            self.action_btn.setStyleSheet("""
                QPushButton { border: 2px solid #EF4444; color: #EF4444; background: transparent; border-radius: 8px; font-weight: bold; }
            """)
            ...
            if self.can_work:
                ...
                self.action_btn.clicked.connect(
                    lambda checked=False: self.on_launch_callback(self.project_root, config_path, self.task_data)
                )
            else:
                ...
                self.action_btn.setEnabled(False)
                self.action_btn.setStyleSheet("""
                    QPushButton:disabled { border: 1px solid #475569; color: #94A3B8; background: transparent; border-radius: 8px; font-weight: bold; font-size: 14px; }
                """)
        else:
    ...
    def _on_thumbnail_ready(self, img_bytes: bytes):
        ...
        if not image.isNull():
            ...
            # Escalado responsivo manteniendo el 16:9 
